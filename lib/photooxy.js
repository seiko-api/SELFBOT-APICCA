...
const tema = {
  ...
  neonGlow: "https://photooxy.com/logo-and-text-effects/make-smoky-neon-glow-effect-343.html",
...
async function pneonGlow(text1) {
  return new Promise((resolve, reject) => {
    const options = { method: 'POST',
      url: tema.neonGlow,
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
...
async function pGraffitiText(text1) {
  return new Promise((resolve, reject) => {
    const options = { method: 'POST',
      url: tema.GraffitiText,
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
      formData: { text_1: text1, login: 'OK' } };
